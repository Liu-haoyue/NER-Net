...
#         self.dataset_name = os.path.basename(options.input_file).split('.')[0]  # 使用文件名命名输出文件夹名称
...
#         cv2.imwrite(join(self.output_folder, 'frame_{:010d}.png'.format(event_tensor_id)), img)
...
def events_to_voxel_grid_pytorch(events, num_bins, width, height, device):
    ...
    with torch.no_grad():
        ...
        with DeviceTimer('Voxel grid voting'):
            ...
            # print(ys[valid_indices] * width)
            # print((tis_long[valid_indices] + 1))
            # print((tis_long[valid_indices] + 1) * width * height)
            # print(xs[valid_indices] + ys[valid_indices] * width + (tis_long[valid_indices] + 1) * width * height)
            # print('---------------------------------------')
            voxel_grid.index_add_(dim=0,
