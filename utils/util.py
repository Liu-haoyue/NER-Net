...
def get_height_width_evs(data_loader):
    for d in data_loader:
        return d['frame'].shape[1], d['frame'].shape[2]
...
def torch2cv2_normalize(image):
    """convert torch tensor to format compatible with cv2.imwrite"""
    image = torch.squeeze(image)  # H x W
    image = image.cpu().numpy()
    image_normalized = (image - np.min(image)) / (np.max(image) - np.min(image))
    # print('max: ', np.max(image_normalized))
    # print('min: ', np.min(image_normalized))
