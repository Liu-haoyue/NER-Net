...
# from data_loader.dataset_with_voxel import MemMapDataset_voxel
...
class BaseVoxelDataset(Dataset):
    ...
    def __getitem__(self, index, seed=None):
        ...
        aug_data = True
        if events.shape[0] == 0:
            events = torch.tensor([[0, 0, 0, 1]], dtype=torch.float32)
            aug_data = False
        ...
        if self.voxel_method['method'] == 'between_frames':
            ...
            frame, cord_param, flip_param = self.transform_frame(frame, seed)

            if self.has_flow:
                flow = self.get_flow(index)
                # convert to displacement (pix) if use simulated data
                # flow = flow * dt
                flow, _, _ = self.transform_flow(flow, seed)
            else:
            ...
            aug_events = events
            if aug_data:
                i, h, j, w = cord_param[0], cord_param[1], cord_param[2], cord_param[3]
                mask = (events[:, 1] >= i) & (events[:, 1] < i+h) & (events[:, 0] >= j) & (events[:, 0] < j+w)

                aug_events = events[mask]
                aug_events[:, 1] = aug_events[:, 1] - i
                aug_events[:, 0] = aug_events[:, 0] - j

                if aug_events.shape[0] == 0:
                    aug_events = torch.tensor([[0, 0, 0, 1]], dtype=torch.float32)

                # flip events
                for flip_dim in flip_param:
                    if flip_dim == -1:
                        # horizontal flip
                        aug_events[:, 0] = w - 1 - aug_events[:, 0]
                    elif flip_dim == -2:
                        # vertical flip
                        aug_events[:, 1] = h - 1 - aug_events[:, 1]
    ...
    def set_voxel_method(self, voxel_method):
        ...
        if self.voxel_method['method'] == 'k_events':
            ...
            if not self.has_event_indices():
                ...
                self.event_indices = self.get_event_indices_npy()
    ...
    def transform_frame(self, frame, seed):
        ...
        if self.transform:
            random.seed(seed)
            frame, cord_param, flip_param = self.transform(frame)

        return frame, cord_param, flip_param
...
class MemMapDataset(BaseVoxelDataset):
    ...
    def get_frame(self, index):
        frame = self.filehandle['images'][index][:, :, 0]  # (n, h, w, 1) --> (h, w)
        return frame

    def get_flow(self, index):
        flow = self.filehandle['optic_flow'][index]  # (n, 2, h, w) --> (2, h, w)
        return flow
    ...
    def get_events(self, idx0, idx1):
        ...
        ps = self.filehandle["p"][idx0:idx1] * 2.0 - 1.0  # convert [False, True] to [-1, 1]
        return xs, ys, ts, ps
    ...
    def get_event_indices_npy(self):
        return self.filehandle["event_indices"]

    # image_fname = "images.npy"
    def load_data(self, data_path, timestamp_fname="timestamps.npy", image_fname="images.npy",
                  optic_flow_fname="flow.npy", optic_flow_stamps_fname="optic_flow_stamps.npy",
                  t_fname="t.npy", xy_fname="xy.npy", p_fname="p.npy", event_indices_fname="image_event_indices.npy"):
        ...
        for subroot, _, fnames in sorted(os.walk(data_path)):
            for fname in sorted(fnames):
                ...
                if fname.endswith(".npy") and 'flow.npy' not in fname:
                    if fname.endswith(timestamp_fname):
                        frame_stamps = np.load(path, mmap_mode="r")
                        data["frame_stamps"] = frame_stamps
                    elif fname.endswith(image_fname):
                        image_npy = np.load(path, mmap_mode="r")
                        if image_npy.ndim == 3:
                            image_npy = np.expand_dims(image_npy, axis=-1)
                        data["images"] = image_npy
                    # elif fname.endswith(optic_flow_fname):
                    #     data["optic_flow"] = np.load(path, mmap_mode="r")
                    #     self.has_flow = True
                    elif fname.endswith(optic_flow_stamps_fname):
                        optic_flow_stamps = np.load(path, mmap_mode="r")
                        data["optic_flow_stamps"] = optic_flow_stamps
                    elif fname.endswith(event_indices_fname):
                        event_indices = np.load(path, mmap_mode="r")
    ...
    def find_config(self, data_path):
        if self.sensor_resolution is None:
            config = os.path.join(data_path, "metadata.json")
            if os.path.exists(config):
                self.config = read_json(config)
                # self.data_source = self.config['data_source']
                self.data_source = 'v2e'
...
class SequenceDataset_evs(Dataset):
    """Load sequences of time-synchronized {event tensors + frames} from a folder."""
    def __init__(self, data_root, sequence_length, use_voxel_input=False, dataset_type='MemMapDataset',
            step_size=None, proba_pause_when_running=0.0,
            proba_pause_when_paused=0.0, normalize_image=False,
            noise_kwargs={}, hot_pixel_kwargs={}, dataset_kwargs={}, inverse_image=False):
        ...
        if use_voxel_input:
            dataset_type = 'MemMapDataset_voxel'
        self.dataset = eval(dataset_type)(data_root, **dataset_kwargs)

        # self.dataset.length: image number
    ...
    def __getitem__(self, i):
        ...
        k = 0
        j = i * self.step_size
        item = self.dataset.__getitem__(j, seed)  # BaseVoxelDataset: {'events': voxel, ...}, 'events' in 'item' is a voxel tensor
        ...
        if self.normalize_image:
            normalize_image_sequence_(sequence, self.inverse_image, key='frame')
