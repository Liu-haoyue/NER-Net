...
class BaseVoxelDataset(Dataset):
    ...
    def __init__(self, data_path, transforms={}, sensor_resolution=None, num_bins=5,
                 ...
                 filter_hot_events=False, noise_kwargs={}):
        """
    ...
    def __getitem__(self, index, seed=None):
        ...
        if len(xs) == 0:
            ...
            evs = torch.zeros(size, dtype=torch.float32)
        else:
            evs = torch.stack([xs, ys, ts, ps], dim=1)
        raw_frame = self.get_frame(index)
        ...
        if self.noise_kwargs != {}:
            voxel = add_noise_to_voxel(voxel, **self.noise_kwargs)
...
class DynamicH5Dataset(BaseVoxelDataset):
    ...
    def load_data(self, data_path):
        ...
        # 有些数据集H5文件中的分辨率顺序为[W, H]，修改为计算体素所需要的[H, W]
